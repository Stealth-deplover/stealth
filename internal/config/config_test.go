...
func TestAgentProviderModelValid(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "valid provider and model", provider: "local", model: "model-a", want: true},
		{name: "model belongs to another provider", provider: "local", model: "model-x"},
		{name: "unknown provider", provider: "missing", model: "model-a"},
		{name: "unknown model", provider: "remote", model: "missing"},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if got := AgentProviderModelValid(catalog, test.provider, test.model); got != test.want {
				t.Fatalf("AgentProviderModelValid(%q, %q) = %v, want %v", test.provider, test.model, got, test.want)
			}
		})
	}
}
