...
func (s *Server) registerSiteRoutes(r chi.Router) {
	...
	r.With(s.requireProjectManagement, s.rateLimitProjectOperation("site_deployment")).Post("/projects/{projectID}/sites/{siteID}/deployments", s.uploadSiteDeployment)
	r.With(s.requireProjectManagement, s.rateLimitProjectOperation("site_deployment_git")).Post("/projects/{projectID}/sites/{siteID}/deployments/git", s.createGitSiteDeployment)
