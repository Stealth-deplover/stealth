...
func createAdminTelemetryTestTables(t *testing.T, ctx context.Context, connection driver.Conn) {
	...
	for _, statement := range []string{
		...
		`CREATE TABLE IF NOT EXISTS otel_metrics_histogram (TimeUnix DateTime, MetricName String, ServiceName String, Attributes Map(String, String), ResourceAttributes Map(String, String), Count UInt64, Sum Float64, BucketCounts Array(UInt64), ExplicitBounds Array(Float64), Min Float64, Max Float64, AggregationTemporality Int32) ENGINE = MergeTree ORDER BY TimeUnix`,
		`CREATE TABLE IF NOT EXISTS otel_metrics_summary (TimeUnix DateTime, MetricName String, ServiceName String, Attributes Map(String, String), ResourceAttributes Map(String, String), Count UInt64, Sum Float64, ValueAtQuantiles Nested(Quantile Array(Float64), Value Array(Float64))) ENGINE = MergeTree ORDER BY TimeUnix`,
		`CREATE TABLE IF NOT EXISTS otel_metrics_exp_histogram (TimeUnix DateTime, MetricName String, ServiceName String, Attributes Map(String, String), ResourceAttributes Map(String, String), Count UInt64, Sum Float64, Scale Int32, ZeroCount UInt64, PositiveOffset Int32, PositiveBucketCounts Array(UInt64), NegativeOffset Int32, NegativeBucketCounts Array(UInt64), Min Float64, Max Float64, AggregationTemporality Int32) ENGINE = MergeTree ORDER BY TimeUnix`,
	} {
