...
| App rollback and diagnostics | Rollback requires `apps.write` and locks the App plus current/target deployment rows in one transaction. It validates the ready older release's immutable artifact metadata and canonical WorkloadSpec snapshot, restores that snapshot, selects the target, advances the desired generation once, resets retry state, and writes safe audit metadata. | The API only commits desired PostgreSQL state. The existing runtime worker claims the new fenced generation, verifies and imports the persisted OCI archive if the Docker cache tag is absent, and requires fresh health before routing. Diagnostics use a PostgreSQL repeatable-read projection shared with the App health/route rules; the API has no Docker call in this path. | Rollback preserves current environment variables/secrets, App name, enabled state, hostname, and quota. Stale pre-rollback worker writes fail generation/lease fencing. Corrupt OCI archives fail closed without an automatic fallback. Diagnostics expose fixed statuses/issues, sanitized runtime error classifications, and bounded retry/timestamp state; container/image IDs, route identities, worker leases, private paths, and secret data are omitted. | Immutable deployment rows and build logs remain unchanged. A6 runtime image GC can remove only the Docker cache tag; the persisted OCI archive remains available for recovery and future rollback. Rollback is operator initiated; runtime failure does not trigger it automatically. |
