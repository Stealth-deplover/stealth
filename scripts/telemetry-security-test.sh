...
if ! printf '%s\n' "$collector_block" | grep -E '^[[:space:]]*-[[:space:]]*DAC_READ_SEARCH[[:space:]]*$' >/dev/null 2>&1; then
	printf 'telemetry security check: main collector lacks its narrow Docker JSON read/search capability\n' >&2
	exit 1
fi
