...
import { maskSecret, OneTimeSecretDialog } from "@/components/one-time-secret-dialog";
...
describe("one-time secret dialog", () => {
  it("masks the value until the user reveals it", () => {
    ...
    expect(screen.getByTestId("one-time-secret")).not.toHaveTextContent("sk_live_123456789");

    fireEvent.click(screen.getByRole("button", { name: "Reveal" }));
    expect(screen.getByTestId("one-time-secret")).toHaveTextContent("sk_live_123456789");
  });
  ...
  it("requires an explicit acknowledgment before clearing the secret", () => {
    ...
    fireEvent.click(screen.getByRole("checkbox", { name: /saved this secret/i }));
