...
export function OrganizationSwitcher({ currentId }: { currentId?: string }) {
  ...
  const organizations = useMemo(() => {
    ...
    return selected &&
      !items.some((organization) => organization.id === selected.id)
      ? [selected, ...items]
      : items;
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Switch the workspace context for this console.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="max-h-72 space-y-1 overflow-y-auto">
          {isLoading ? (
            ...
            filtered.map((organization) => (
              <SelectorItem
                key={organization.id}
                icon={getInitials(organization.name)}
                title={organization.name}
                subtitle={organization.slug}
                selected={organization.id === currentId}
...
export function ProjectSwitcher({
  ...
  const projects = useMemo(() => {
    ...
    return selected &&
      ...
      !items.some((project) => project.id === selected.id)
      ? [selected, ...items]
      : items;
