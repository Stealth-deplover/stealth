...
function SelectorItem({ icon, title, subtitle, selected, onClick }: { icon: React.ReactNode; title: string; subtitle?: string; selected?: boolean; onClick: () => void }) {
  return <button type="button" onClick={onClick} className={cn("flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left transition hover:bg-white/[0.06]", selected && "bg-cyan-300/10")}>
    ...
    {selected ? <span className="size-1.5 rounded-full bg-cyan-300" /> : null}
  </button>;
}
...
export function ProjectSwitcher({ organizationId, currentId }: { organizationId?: string; currentId?: string }) {
  ...
  return <Dialog open={open} onOpenChange={setOpen}>
    <DialogTrigger asChild><Button variant="ghost" className="h-10 max-w-60 justify-start border-l border-stealth-border pl-3 pr-2.5"><span className="flex size-7 items-center justify-center rounded-md bg-violet-300/10 text-violet-200"><FolderKanban className="size-3.5" /></span><span className="min-w-0 flex-1 text-left"><span className="block truncate text-xs font-semibold text-white">{current?.name ?? "Select project"}</span><span className="block truncate text-[10px] text-slate-500">Project</span></span><ChevronsUpDown className="size-3.5 text-slate-500" /></Button></DialogTrigger>
