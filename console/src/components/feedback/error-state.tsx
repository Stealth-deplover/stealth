...
export function ErrorState({ error, retry, title = "Unable to load this view" }: { error: unknown; retry?: () => void; title?: string }) {
  return <Card className="border-rose-300/20 bg-rose-400/[0.04]" role="alert">
    <CardHeader><CardTitle className="flex items-center gap-2 text-rose-200"><AlertTriangle className="size-4" /> {title}</CardTitle></CardHeader>
