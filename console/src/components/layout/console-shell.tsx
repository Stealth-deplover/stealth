...
export function ConsoleShell({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  useEffect(() => { if (account.error && "status" in account.error && Number(account.error.status) === 401) router.replace("/login"); }, [account.error, router]);
  ...
  if (account.error && !("status" in account.error && Number(account.error.status) === 401)) return <main className="mx-auto flex min-h-screen max-w-2xl items-center px-6"><ErrorState error={account.error} retry={() => account.refetch()} /></main>;

  return <div className="min-h-screen bg-stealth-bg"><div className="flex min-h-screen"><Sidebar {...context} />{mobileOpen ? <div className="fixed inset-0 z-40 bg-black/70 lg:hidden" onClick={() => setMobileOpen(false)}><div className="h-full w-72" onClick={(event) => event.stopPropagation()}><Sidebar {...context} /></div></div> : null}<div className="min-w-0 flex-1"><Topbar {...context} onMenu={() => setMobileOpen(true)} /><main className="mx-auto w-full max-w-[1600px] px-4 py-7 sm:px-6 lg:px-10">{children}</main></div></div></div>;
}
