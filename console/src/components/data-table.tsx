...
import { CursorPaginationControls, type CursorPaginationControlsProps } from "@/components/cursor-pagination-controls";
...
export function DataTable<T>({ columns, data, loading, empty = "No records yet.", serverPagination }: DataTableProps<T>) {
  ...
  return <div>
    ...
    {!loading && serverPagination ? <CursorPaginationControls {...serverPagination} label={serverPagination.label ?? `${data.length} records on this page`} /> : !loading && table.getPageCount() > 1 ? <div className="flex flex-wrap items-center justify-between gap-3 border-t border-stealth-border px-4 py-3 text-xs text-slate-500"><span>Showing {pagination.pageIndex * pagination.pageSize + 1}–{Math.min((pagination.pageIndex + 1) * pagination.pageSize, data.length)} of {data.length}</span><div className="flex items-center gap-2"><button type="button" className="rounded-md border border-stealth-border px-2.5 py-1.5 transition hover:bg-white/[0.05] disabled:cursor-not-allowed disabled:opacity-40" disabled={!table.getCanPreviousPage()} onClick={() => table.previousPage()}>Previous</button><span>Page {pagination.pageIndex + 1} / {table.getPageCount()}</span><button type="button" className="rounded-md border border-stealth-border px-2.5 py-1.5 transition hover:bg-white/[0.05] disabled:cursor-not-allowed disabled:opacity-40" disabled={!table.getCanNextPage()} onClick={() => table.nextPage()}>Next</button></div></div> : null}
  </div>;
}
