...
import { Background, Controls, MiniMap, Panel, ReactFlow, useEdgesState, useNodesState, type Node, type NodeProps } from "@xyflow/react";
import { ArrowUpRight, Database, FileArchive, FunctionSquare, HardDrive, Save, Waypoints } from "lucide-react";
...
function ResourceNode({ data }: NodeProps<ResourceNode>) {
  ...
  return <div className={`min-w-48 rounded-xl border px-4 py-3 shadow-xl ${resourceAccent[data.type]}`}><div className="flex items-start gap-3"><span className="mt-0.5 flex size-7 items-center justify-center rounded-lg bg-black/20"><Icon className="size-4" /></span><div className="min-w-0"><p className="truncate text-sm font-semibold">{data.label}</p><p className="mt-0.5 truncate text-[11px] opacity-70">{data.subtitle ?? data.type}</p></div></div>{data.status ? <div className="mt-3"><StatusBadge status={data.status} /></div> : null}</div>;
}

const nodeTypes = { resource: ResourceNode };

export function ServicesCanvasView({ organizationId, projectId }: { organizationId: string; projectId: string }) {
  ...
  const resourceLink = selected ? (() => { const [type, resourceId] = selected.id.split(":"); const base = `/organizations/${organizationId}/projects/${projectId}`; return type === "function" ? `${base}/functions/${resourceId}` : type === "site" ? `${base}/sites/${resourceId}` : type === "database" ? `${base}/databases/${resourceId}` : `${base}/storage/${resourceId}`; })() : undefined;
  ...
  return <><PageHeader eyebrow="Topology" title="Services" description="A durable map of every resource returned by the cursor-paginated APIs. Relationships are intentionally absent until the backend exposes explicit dependency edges." actions={layout.data?.can_manage ? <Button onClick={saveLayout} disabled={save.isPending || loading}><Save className="size-4" /> Save positions</Button> : null} /><div className="mb-4 flex flex-wrap items-center gap-2"><Badge variant="neutral"><Waypoints className="size-3" /> {nodes.length} nodes</Badge><span className="text-xs text-slate-600">Resource layout only. Drag, zoom, pan, and fit view; no synthetic edges.</span></div>{loading ? <Card className="flex min-h-[420px] items-center justify-center"><LoadingState rows={3} /></Card> : nodes.length === 0 ? <EmptyState icon={<Waypoints className="size-5" />} title="No services yet" description="Create a Function, Site, Database, or Storage resource and it will appear here. The canvas only renders resources returned by the API." /> : <Card className="h-[calc(100vh-15rem)] min-h-[460px] overflow-hidden"><ReactFlow nodes={nodes} edges={edges} nodeTypes={nodeTypes} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onNodeClick={(_, node) => setSelectedId(node.id)} onPaneClick={() => setSelectedId(null)} fitView proOptions={{ hideAttribution: true }} className="bg-stealth-bg"><Background color="var(--border)" gap={28} size={1} /><Controls /><MiniMap nodeColor={(node) => node.data?.type === "function" ? "#72e4dc" : node.data?.type === "site" ? "#b9a1ff" : node.data?.type === "database" ? "#f2c66d" : "#73d6a2"} /><Panel position="top-right" className="rounded-lg border border-stealth-border bg-stealth-elevated/90 px-3 py-2 text-xs text-slate-500">Resource layout only · backend-owned</Panel>{selected ? <Panel position="bottom-right" className="w-64 rounded-xl border border-stealth-border bg-stealth-elevated/95 p-4 shadow-2xl"><p className="text-[10px] uppercase tracking-[0.16em] text-slate-600">Selected resource</p><p className="mt-2 truncate text-sm font-semibold text-white">{selected.data.label}</p><p className="mt-1 text-xs capitalize text-slate-500">{selected.data.type}{selected.data.subtitle ? ` · ${selected.data.subtitle}` : ""}</p>{selected.data.status ? <div className="mt-3"><StatusBadge status={selected.data.status} /></div> : null}<p className="mt-3 text-[11px] leading-5 text-slate-600">Edges are withheld until the API reports a real dependency.</p>{resourceLink ? <Button asChild variant="ghost" size="sm" className="mt-3 w-full justify-between"><Link href={resourceLink}>Open resource <ArrowUpRight className="size-3.5" /></Link></Button> : null}</Panel> : null}</ReactFlow></Card>}</>;
}
