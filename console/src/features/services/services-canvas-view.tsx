...
import { useFunctions, useDatabases, useServiceLayout, useSites, useStorageBuckets } from "@/api/queries";
...
export function ServicesCanvasView({ projectId }: { projectId: string }) {
  ...
  const initialNodes = useMemo<ResourceNode[]>(() => {
    ...
    functions.data?.functions.forEach((item, index) => { const saved = layoutMap.get(`function:${item.id}`); nodes.push({ id: `function:${item.id}`, type: "resource", position: { x: saved?.x ?? 80 + (index % 3) * 250, y: saved?.y ?? 80 + Math.floor(index / 3) * 180 }, data: { label: item.name, type: "function", status: item.status, subtitle: item.runtime } }); });
    sites.data?.sites.forEach((item, index) => { const saved = layoutMap.get(`site:${item.id}`); nodes.push({ id: `site:${item.id}`, type: "resource", position: { x: saved?.x ?? 80 + (index % 3) * 250, y: saved?.y ?? 360 + Math.floor(index / 3) * 180 }, data: { label: item.name, type: "site", status: item.status, subtitle: item.framework } }); });
    databases.data?.databases.forEach((item, index) => { const saved = layoutMap.get(`database:${item.id}`); nodes.push({ id: `database:${item.id}`, type: "resource", position: { x: saved?.x ?? 80 + (index % 3) * 250, y: saved?.y ?? 640 + Math.floor(index / 3) * 180 }, data: { label: item.name, type: "database", subtitle: "typed data" } }); });
    buckets.data?.buckets.forEach((item, index) => { const saved = layoutMap.get(`storage:${item.id}`); nodes.push({ id: `storage:${item.id}`, type: "resource", position: { x: saved?.x ?? 80 + (index % 3) * 250, y: saved?.y ?? 920 + Math.floor(index / 3) * 180 }, data: { label: item.name, type: "storage", subtitle: "object storage" } }); });
    return nodes;
  }, [buckets.data?.buckets, databases.data?.databases, functions.data?.functions, layoutMap, sites.data?.sites]);
  ...
  return <><PageHeader eyebrow="Topology" title="Services" description="A durable map of project resources. Relationships are intentionally absent until the backend exposes explicit dependency edges." actions={layout.data?.can_manage ? <Button onClick={saveLayout} disabled={save.isPending}><Save className="size-4" /> Save positions</Button> : null} /><div className="mb-4 flex items-center gap-2"><Badge variant="neutral"><Waypoints className="size-3" /> {nodes.length} nodes</Badge><span className="text-xs text-slate-600">Drag, zoom, pan, and fit view. No synthetic edges.</span></div><Card className="h-[calc(100vh-15rem)] min-h-[520px] overflow-hidden"><ReactFlow nodes={nodes} edges={edges} nodeTypes={nodeTypes} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} fitView proOptions={{ hideAttribution: true }} className="bg-[#090c10]"><Background color="#27303c" gap={28} size={1} /><Controls /><MiniMap nodeColor={(node) => node.data?.type === "function" ? "#72e4dc" : node.data?.type === "site" ? "#b9a1ff" : node.data?.type === "database" ? "#f2c66d" : "#73d6a2"} /><Panel position="top-right" className="rounded-lg border border-stealth-border bg-stealth-elevated/90 px-3 py-2 text-xs text-slate-500">Resource layout only · backend-owned</Panel></ReactFlow></Card></>;
}
