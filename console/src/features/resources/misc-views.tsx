...
import { useAgent, useAgentCatalog, useAgents, useAuthSettings, useFunctions, useProject, useProjectTraces, useSites, useMessagingMessages, useMessagingProviders, useMessagingTopics } from "@/api/queries";
...
export function DeploymentsView({ organizationId, projectId }: { organizationId: string; projectId: string }) {
  const functionsNavigation = useCursorPagination("deployment_functions_cursor");
  const sitesNavigation = useCursorPagination("deployment_sites_cursor");
  const functions = useFunctions(projectId, { cursor: functionsNavigation.cursor });
  ...
  return <><PageHeader eyebrow="Compute" title="Deployments" description="The backend exposes deployment history under each Function and Site. This page links the actual resource-scoped views instead of inventing a global deployment endpoint." /><div className="grid gap-4 md:grid-cols-2"><Card><CardHeader><CardTitle>Functions</CardTitle><p className="mt-1 text-xs text-slate-500">Functions on this API page.</p></CardHeader><CardContent className="space-y-3">{functions.data?.functions.length ? functions.data.functions.map((item) => <Link key={item.id} href={`${base}/functions/${item.id}`} className="flex items-center justify-between rounded-lg border border-stealth-border p-3 hover:border-cyan-300/30"><span><span className="block text-sm font-medium text-white">{item.name}</span><span className="text-xs text-slate-600">{item.active_deployment_id ? "Active deployment available" : "No active deployment"}</span></span><StatusBadge status={item.active_deployment_id ? "active" : "inactive"} /></Link>) : <p className="text-sm text-slate-500">No functions returned.</p>}</CardContent><CursorPaginationControls canPrevious={functionsNavigation.canPrevious} canNext={Boolean(nextCursor(functions.data))} onPrevious={functionsNavigation.goPrevious} onNext={() => functionsNavigation.goNext(nextCursor(functions.data))} isFetching={functions.isFetching} label="Function page" /></Card><Card><CardHeader><CardTitle>Sites</CardTitle><p className="mt-1 text-xs text-slate-500">Sites on this API page.</p></CardHeader><CardContent className="space-y-3">{sites.data?.sites.length ? sites.data.sites.map((item) => <Link key={item.id} href={`${base}/sites/${item.id}`} className="flex items-center justify-between rounded-lg border border-stealth-border p-3 hover:border-violet-300/30"><span><span className="block text-sm font-medium text-white">{item.name}</span><span className="text-xs text-slate-600">{item.active_deployment_id ? "Active deployment available" : "No active deployment"}</span></span><StatusBadge status={item.active_deployment_id ? "active" : "inactive"} /></Link>) : <p className="text-sm text-slate-500">No sites returned.</p>}</CardContent><CursorPaginationControls canPrevious={sitesNavigation.canPrevious} canNext={Boolean(nextCursor(sites.data))} onPrevious={sitesNavigation.goPrevious} onNext={() => sitesNavigation.goNext(nextCursor(sites.data))} isFetching={sites.isFetching} label="Site page" /></Card></div></>;
}
...
export function LogsView({ organizationId, projectId }: { organizationId: string; projectId: string }) {
  ...
  const sites = useSites(projectId, { cursor: sitesNavigation.cursor });
  if (functions.error || sites.error) return <ErrorState error={functions.error ?? sites.error} retry={() => { void functions.refetch(); void sites.refetch(); }} />;
  return <><PageHeader eyebrow="Observability" title="Logs" description="Logs are contextual to Function builds/executions, Site builds, and Agent runs. Pick a resource to preserve the backend's incremental cursor semantics." /><div className="grid gap-4 md:grid-cols-3">{functions.data?.functions.map((item) => <Card key={item.id}><CardContent className="p-5"><FunctionSquareIcon /><h2 className="mt-4 text-sm font-semibold text-white">{item.name}</h2><p className="mt-1 text-xs leading-5 text-slate-500">Build and execution logs are available from the Function detail view.</p><Link href={`/organizations/${organizationId}/projects/${projectId}/functions/${item.id}`} className="mt-4 inline-flex text-xs text-cyan-300">Open function →</Link></CardContent></Card>)}{sites.data?.sites.map((item) => <Card key={item.id}><CardContent className="p-5"><TerminalSquare className="size-5 text-violet-300" /><h2 className="mt-4 text-sm font-semibold text-white">{item.name}</h2><p className="mt-1 text-xs leading-5 text-slate-500">Site build logs are scoped to each site deployment.</p><Link href={`/organizations/${organizationId}/projects/${projectId}/sites/${item.id}`} className="mt-4 inline-flex text-xs text-violet-300">Open site →</Link></CardContent></Card>)}</div><div className="mt-4 grid gap-4 md:grid-cols-2"><Card><CursorPaginationControls canPrevious={functionsNavigation.canPrevious} canNext={Boolean(nextCursor(functions.data))} onPrevious={functionsNavigation.goPrevious} onNext={() => functionsNavigation.goNext(nextCursor(functions.data))} isFetching={functions.isFetching} label="Function log sources" /></Card><Card><CursorPaginationControls canPrevious={sitesNavigation.canPrevious} canNext={Boolean(nextCursor(sites.data))} onPrevious={sitesNavigation.goPrevious} onNext={() => sitesNavigation.goNext(nextCursor(sites.data))} isFetching={sites.isFetching} label="Site log sources" /></Card></div>{!functions.data?.functions.length && !sites.data?.sites.length ? <div className="mt-4"><EmptyState title="No log sources yet" description="Create a Function, Site, or Agent run to get a backend-owned incremental log stream." /></div> : null}</>;
}
...
export function AgentsView({ organizationId, projectId }: { organizationId: string; projectId: string }) {
  ...
  const roleOptions = (catalog.data?.roles ?? []).map((role) => ({ value: role, label: role }));
  const providerOptions = (catalog.data?.providers ?? []).map((item) => ({ value: item.id, label: item.name }));
  const modelOptions = [...new Set((catalog.data?.providers ?? []).flatMap((item) => item.models))].map((model) => ({ value: model, label: model }));
  const catalogReady = Boolean(catalog.data && provider && roleOptions.length && providerOptions.length && modelOptions.length);
...
export function MessagingView({ projectId }: { projectId: string }) {
  ...
  const providers = useMessagingProviders(projectId, { cursor: providersNavigation.cursor });
  const topics = useMessagingTopics(projectId, { cursor: topicsNavigation.cursor });
  const messages = useMessagingMessages(projectId, { cursor: messagesNavigation.cursor });
  ...
  return <><PageHeader eyebrow="Integrations" title="Messaging" description="Provider credentials and message content stay protected by the backend. The console exposes safe metadata only." /><div className="grid gap-4 md:grid-cols-3"><Card><CardContent className="p-5"><p className="text-xs text-slate-500">Providers on this page</p><p className="mt-2 text-2xl font-semibold text-white">{providers.data?.providers.length ?? "—"}</p><p className="mt-2 text-xs text-slate-600">Encrypted credentials are never returned.</p><CursorPaginationControls {...{ canPrevious: providersNavigation.canPrevious, canNext: Boolean(nextCursor(providers.data)), onPrevious: providersNavigation.goPrevious, onNext: () => providersNavigation.goNext(nextCursor(providers.data)), isFetching: providers.isFetching }} label="Provider page" /></CardContent></Card><Card><CardContent className="p-5"><p className="text-xs text-slate-500">Topics on this page</p><p className="mt-2 text-2xl font-semibold text-white">{topics.data?.topics.length ?? "—"}</p><CursorPaginationControls {...{ canPrevious: topicsNavigation.canPrevious, canNext: Boolean(nextCursor(topics.data)), onPrevious: topicsNavigation.goPrevious, onNext: () => topicsNavigation.goNext(nextCursor(topics.data)), isFetching: topics.isFetching }} label="Topic page" /></CardContent></Card><Card><CardContent className="p-5"><p className="text-xs text-slate-500">Messages on this page</p><p className="mt-2 text-2xl font-semibold text-white">{messages.data?.messages.length ?? "—"}</p><p className="mt-2 text-xs text-slate-600">Metadata only; content remains encrypted.</p><CursorPaginationControls {...{ canPrevious: messagesNavigation.canPrevious, canNext: Boolean(nextCursor(messages.data)), onPrevious: messagesNavigation.goPrevious, onNext: () => messagesNavigation.goNext(nextCursor(messages.data)), isFetching: messages.isFetching }} label="Message page" /></CardContent></Card></div><Card className="mt-5"><CardHeader><CardTitle>Topics</CardTitle></CardHeader><CardContent className="space-y-2">{topics.data?.topics.length ? topics.data.topics.map((topic) => <div key={topic.id} className="flex items-center justify-between rounded-lg border border-stealth-border px-3 py-3"><div><p className="text-sm font-medium text-white">{topic.name}</p><p className="text-xs text-slate-600">{topic.subscriber_count} subscribers</p></div><StatusBadge status={topic.enabled ? "active" : "inactive"} /></div>) : <p className="text-sm text-slate-500">No topics returned.</p>}</CardContent></Card></>;
}
