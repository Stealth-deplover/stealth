...
function AppEditorForm({
  ...
  return (
    <>
      <DialogHeader>
        <DialogTitle>
          {app ? "Edit App configuration" : "Create an App"}
        </DialogTitle>
        <DialogDescription>
          Set durable runtime intent. WorkloadSpec values left blank use the
          server’s v1 defaults.
        </DialogDescription>
      </DialogHeader>
      <form onSubmit={submit} className="space-y-5">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          >
            <Input
              ...
              onChange={(event) =>
                update("working_directory", event.target.value)
              }
              placeholder="/srv/app"
              autoComplete="off"
            />
        ...
        >
          <Textarea
            ...
            onChange={(event) =>
              update("command_arguments", event.target.value)
            }
            placeholder={"./server\n--port\n8080"}
            spellCheck={false}
