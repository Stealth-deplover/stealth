...
export function useCursorPagination(param = "cursor") {
  ...
  }>({ contextKey, history: [currentCursor] });
  ...
  const history =
    paginationState.contextKey === contextKey
  ...
  const goNext = (next: string | null | undefined) => {
    ...
    setPaginationState((current) => {
      const currentHistory =
        current.contextKey === contextKey ? current.history : [currentCursor];
      return {
        contextKey,
        history: appendCursorHistory(currentHistory, currentCursor, next),
      };
    });
    navigate(next);
  };
