...
const functionPageOne = { id: "function-1", project_id: project.id, name: "worker-page-one", runtime: "node-22", entrypoint: "src/index.main", commands: "", timeout_seconds: 15, enabled: true, logging: true, execute_permissions: [], status: "active", artifact_quota_bytes: 100000, artifact_used_bytes: 0, active_deployment_id: null, created_at: "2026-01-01T00:00:00Z", updated_at: "2026-01-01T00:00:00Z" };
const functionPageTwo = { ...functionPageOne, id: "function-2", name: "worker-page-two" };
...
async function installApiFixtures(page: Page) {
  await page.route("**/v1/**", async (route) => {
    ...
    if (path === "/v1/sessions/email-password" && request.method() === "POST") return route.fulfill({ status: 204 });
    if (path === "/v1/session" && request.method() === "DELETE") return route.fulfill({ status: 204 });
    ...
    if (path === "/v1/projects/project-1/functions") {
      ...
      return respond({ functions: [cursor === "cursor-page-2" ? functionPageTwo : functionPageOne], pagination: { limit: 20, next_cursor: cursor === "cursor-page-2" ? null : "cursor-page-2" }, can_manage: false });
    }
...
test("critical console flow can move from login to a resource and logout", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Organizations", exact: true, level: 1 })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "Functions", exact: true, level: 1 })).toBeVisible();
...
test("cursor pagination navigates next and previous server pages", async ({ page }) => {
  await installApiFixtures(page);

  await gotoWithDevRetry(page, "/organizations/org-1/projects/project-1/functions");
...
test("cursor deep links expose First without inventing Previous", async ({ page }) => {
  await installApiFixtures(page);

  await gotoWithDevRetry(page, "/organizations/org-1/projects/project-1/functions?status=active&cursor=cursor-page-2");
  await expect(page.getByText("worker-page-two")).toBeVisible();
  await expect(page.getByRole("button", { name: "Previous" })).toBeDisabled();
